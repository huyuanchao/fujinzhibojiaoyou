...
<div class="bodybox">
    ...
    <div class="head mainBg">
        ...
        <div class="title">{if $id==1}今天{else}{if $id==3}3天内(不包含今天){else}7天内(不包含3天){/if}{/if}新注册用户</div>
	...
	{loop $newyh $x}
		<div class="conDiv">
			<div class="ovfl conLeft">
				...
				<div class="floR rightDiv txtAC">{php echo date('n-j G:i',$x['createtime'])}</div>
			</div>
			<div class="ovfl tipDiv">
				<a href="javascript:void(0)" onClick="send({$x['id']})" class="floL txtAC">快捷发消息</a>
				<a href="{php echo $this->createMobileUrl('dy_send',array('yhid'=>$x['id']))}" class="floL txtAC">和Ta聊天</a>
			</div>
    ...
    <div class="addUser"><a href="javascript:void(0)" onClick="send2({$id})">一键调戏</a></div>
</div>
...
<script type="text/javascript">
	...
	function send2(day)
	{
		if(day)
		{
			$.post("{php echo $_W['siteroot'].'app/'.substr($this->createMobileUrl('dy_kjmsg'),2)}"+"&day="+day,function(data){
	            if (data == 1) {
	            	$.tips("发送成功！");
