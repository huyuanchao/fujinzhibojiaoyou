...
<div class="bodybox">
    <div class="selectedDiv">
        ...
        <a href="{php echo $this->createMobileUrl('dy_man')}" class="rBor mainColor"><img src="../addons/jy_ppp/images/manager.png"><span>管理</span></a>
    </div>
...
<!--         <a href="javascript:history.go(-1)" class="bn pull-left"><img class="icon" src="../addons/jy_ppp/images/back.png"></a> -->
        <div class="title">{if !empty($userid)}关注[{$user_member['nicheng']}]的{else}关注我的{/if}</div>
<!--         <a href="#" class="bn pull-right"><img class="icon" src="../addons/jy_ppp/images/ren.png"/></a> -->
    ...
    <div class="con-con">
        ...
        <div class="con" >
            ...
            {/if}
            <div>
                <div>{$l['nicheng']}<span class="times font13 floR">{php echo date("Y-m-d H:i",$l['createtime'])}</span></div>
                <div class="font13 gray ovfl">{if !empty($l['brith'])}
                    ...
                    {$nianlin}岁·{/if}{$province[$l['province']]}{if !empty($l['height'])}·{$l['height']}cm{/if}<span class="read white mainBg floR">{$l['nicheng2']}</span></div>
            </div>
        </div>
        {/loop}
        <!-- end -->
...
function changeSel(){
    ...
    if(str==0)
    {
        window.location.href="{php echo $_W['siteroot'].'app/'.substr($this->createMobileUrl('dy_man'),2)}";
    }
    else
    {
        window.location.href="{php echo $_W['siteroot'].'app/'.substr($this->createMobileUrl('dy_man'),2)}"+"&userid="+str;
    }
}
