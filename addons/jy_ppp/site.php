...
class Jy_pppModuleSite extends WeModuleSite
{
    ...
    public function doMobileNotify (){
        global $_W, $_GPC;
        $weid = $_W['uniacid'];
        if (empty($weid)) {
        ...
        // print_r($_POST);
        $logs = pdo_fetch('SELECT * FROM ' . tablename('jy_ppp_pay_log') . ' WHERE id=' .$pay_id." and status=0" );
        // echo $price;
        if($logs){
            $credit = pdo_fetchcolumn('SELECT credit FROM ' . tablename('jy_ppp_member') . ' WHERE id=' .$logs['mid'] );
            // echo $credit+$price;
            pdo_update('jy_ppp_member', array('credit' =>$credit+$price), array('id' => $logs['mid']));
            // pdo_update('jy_ppp_pay_log', array('status' =>'1'), array('id' => $logs['id']));
            echo "ok";
        }else{
            header("Location:/");
        }
    ...
    public function faxin()
    {
        ...
        foreach ($mid_arr as $key => $value2) {
            ...
            if (count($xinxi_temp) < 7) {
                ...
                $title2 = "附近的'" . $xinxi_yh['nicheng'] . "'给您发了一封表白信";
                ...
                $data3 = array('weid' => $weid, 'type' => 'news', 'sendid' => $xinxi_yh['sendid'], 'content' => $title2, 'desc' => '', 'picurl' => '', 'status' => $errcode['errcode'], 'createtime' => TIMESTAMP);
                        ...
                        if ($member['sex'] == 2) {
                            ...
                            switch ($rand_wt) {
                                ...
                                case 16:
                                    ...
                                    break;
                                case 17:
                                    $data['content'] = "hey，我觉得你的打扮很好看，可以认识你吗？";
                                    break;
                                default:
                                    $data['content'] = "hello,您好！很高兴认识你，可以交个朋友么？";
                                    break;
                    ...
                    if (!empty($temp) && $temp['createtime'] > $today) {
                        ...
                        if ($kefu_member['type'] != 3 && !empty($kefu_member['from_user'])) {
                            ...
                            if ($send_kefu_tf == true) {
                                ...
                                $errcode = json_decode($response['content'], true);
                                $data3 = array('weid' => $weid, 'mid' => $id, 'sendid' => $send_member['id'], 'type' => 'news', 'content' => $text2, 'desc' => $desc2, 'url' => $url2, 'picurl' => $pic2, 'status' => $errcode['errcode'], 'createtime' => TIMESTAMP);
                                pdo_insert("jy_ppp_kefu", $data3);
                            }
                        }
                    }
                        ...
                        if (!empty($temp) && $temp['createtime'] > $today) {
                            ...
                            if ($kefu_member['type'] != 3 && !empty($kefu_member['from_user'])) {
                                ...
                                if ($send_kefu_tf == true) {
                                    ...
                                    $response = ihttp_request($url, urldecode(json_encode($data)));
                                    $errcode = json_decode($response['content'], true);
                                    $data3 = array('weid' => $weid, 'mid' => $value, 'sendid' => $send_member['id'], 'type' => 'news', 'content' => $text2, 'desc' => $desc2, 'url' => $url2, 'picurl' => $pic2, 'status' => $errcode['errcode'], 'createtime' => TIMESTAMP);
                                    pdo_insert("jy_ppp_kefu", $data3);
                                }
                            }
                        }
    ...
    public function doMobileChat()
    {
        ...
        if (!empty($member['id'])) {
            ...
            if ($op == 'zhaohu') {
                ...
                // echo "SELECT sendid FROM " . tablename('jy_ppp_xinxi') . "  WHERE weid=" . $weid . " AND mid=" . $sendid . " AND sendid=".$sendid." GROUP BY sendid ORDER BY yuedu ASC,createtime DESC LIMIT 1 ";die;
                $end = pdo_fetch("SELECT * FROM " . tablename('jy_ppp_xinxi') . "  WHERE weid=" . $weid . " AND mid=" . $mid . " AND sendid=".$sendid."  ORDER BY createtime DESC LIMIT 1 ");
                if (!empty($end)) {
                    $result=array(
                        'code'=>0,
                        'msg'=>'',
                        'data'=>array(
                            'createtime'=>date("Y-m-d H:i",$end['createtime']),
                            'msgid'=>$end['id'],
                            'leixing'=>$end['leixing'],
                            'content'=>$end['content'],
                            'picurl'=>$end['picurl'],
                            'media_id'=>$end['media_id']
                        )
                    );
                } else {
                    $result=array(
